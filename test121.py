...
def main():
    #im = Image.open(os.path.join(input, image_name)).convert('RGB')
    left = cv2.imread('b_t.jpg')
    #left=cv2.resize(left,dsize=(512,512))
    left_gray = cv2.cvtColor(left, cv2.COLOR_BGR2GRAY)
    right = cv2.imread('b.jpg')
